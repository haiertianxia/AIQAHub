...
### Governance Center
Expose Playwright execution data through the existing governance event stream by relying on the existing governance projection only. The governance center should show only fields that already exist in that projection, with Playwright metadata appearing as read-only values inside those existing rows when present:
- existing row kind / action
- existing row status
- existing row target identifiers
- existing row adapter context
- existing row execution identifier only when it is already present in the current projection

The governance view should not create a new Playwright-only data model, a new subsection/tab, or new Playwright-specific outcome categories. It should simply surface Playwright-related governance rows alongside the existing governance stream, and omit missing context rather than inventing new fields.
